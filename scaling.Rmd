...
```{r}
tweets <- tweets %>%
  sample_n(20000) #selects a random sample of size 20,000 from the data frame.
```
...
dfm_tweets <- dfm(tokens(tweets_corpus,
  ...
  dfm_select(pattern = stopwords("english"), 
             selection = "remove", #remove English stopwords from the dfm
             valuetype = "fixed") #the values in the dfm will be fixed integers
```
...
```{r}
# compress the document-feature matrix at the newspaper level:generate a new dfm named dfm_newsteweets grouping the original dataset dfm_tweets by "newspaper" -- to aggregate the word counts across all tweets from each newspaper separately.
dfm_newstweets <- dfm_group(dfm_tweets, groups = newspaper)

# remove words not used by two or more newspapers:removes words that appear in fewer than two tweets -- trims the dfm based on document frequency.
...
dfm_newstweets_results <- textmodel_wordfish(dfm_newstweets, 
                                             sparse = TRUE) #specifies that the matrix is in sparse format
#textmodel_wordfish(): used to fit a Wordfish model to the input document-feature matrix, which is used for analyzing the ideological positioning of tweets based on the language used.
...
feat_betas %>%
  arrange(desc(betas)) %>%  #arrange in descending order 
  top_n(20) %>%             #selects the top 20 rows 
  kbl() %>%                 #converts the resulting data frame into a "knitr" table
  kable_styling(bootstrap_options = "striped") #adds striped rows to the table using Bootstrap styling options
...
```{r}
#get the estimates of the latent newspaper position
textplot_scale1d(kaneko_dfm_results)
```
The estimated scores got by newspaper are grouped by the influential power in Japan. Specifically, Most newspapers with high levels of influential power in Japan have a similarly high estimated scores at the right hand side of this latent dimension,except the Asahi, Which is a leading newspaper but got an approximately -1 value. However, the tabloid newspapers got relatively lower estimated scores at the left hand side of this latent dimension.

```{r}
#plot the word-level beta
textplot_scale1d(kaneko_dfm_results, margin = "features")
```

```{r}

feature <- kaneko_dfm_results[["features"]]

beta <- kaneko_dfm_results[["beta"]]

feat_beta <- as.data.frame(cbind(feature, beta))
feat_beta$beta <- as.numeric(feat_beta$beta)

feat_beta %>%
  arrange(desc(beta)) %>%  #arrange in descending order 
  top_n(20) %>%             #selects the top 20 rows 
  kbl() %>%                 #converts the resulting data frame into a "knitr" table
  kable_styling(bootstrap_options = "striped") #adds striped rows to the table using Bootstrap styling options
